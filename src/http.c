    ...
    if (!is_online()) {
        ...
        if (!(mac = arp_get(r->clientAddr))) {
            ...
            debug(LOG_INFO, "Failed to retrieve MAC address for ip %s, so not putting in the login request",
                  r->clientAddr);
            safe_asprintf(&urlFragment, "?gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&url=%s",
                          //auth_server->authserv_login_script_path_fragment, 
						  config->gw_address, config->gw_port,
                          config->gw_id, r->clientAddr, url);
        } else {
            debug(LOG_INFO, "Got client MAC address for ip %s: %s", r->clientAddr, mac);
            safe_asprintf(&urlFragment, "?gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&mac=%s&url=%s",
                          //auth_server->authserv_login_script_path_fragment,
    ...
    if ((token = httpdGetVariableByName(r, "token"))) {
        ...
        if (!(mac = arp_get(r->clientAddr))) {
            ...
            if ((client = client_list_find(r->clientAddr, mac)) == NULL) {
				...
				client_mobile = httpdGetVariableByName(r,"mobile");
				if(client_mobile != NULL){
					memcpy(uid,client_mobile->value,strlen(client_mobile->value));
					debug(LOG_DEBUG, "%s %d uid:%s \n",__FUNCTION__,__LINE__,uid);
					client_list_add(r->clientAddr, mac, token->value,uid);
				}
